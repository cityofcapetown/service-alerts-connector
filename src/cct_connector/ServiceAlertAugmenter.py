...
def _cptgpt_call_wrapper(message_dict: typing.Dict, http_session: requests.Session,
                         max_post_length: int, hashtags=False) -> str or None:
    system_prompt = (
    ...
    params = {
        ...
        "messages": [
            ...
            {"role": "assistant", "content": (
                ...
                "expected by 1pm, 23 Sep. For more info, contact City with request number 9115677540 #CapeTown #Electricity"
            )},
            # {"role": "user", "content": (
    ...
    for t in range(3):
        try:
            ...
            expected_response_tokens = int((max_post_length // 4) * 1.4)
            ...
            params["messages"] = [
                ...
                {"role": "user", "content": (
                    ...
                    'serviced. Take bin onto property & place out by 06:30 the following day. #CTInfo #CapeTown #Refuse'
                )},
                {"role": "assistant", "content": (
                    '🚮Refuse collection delays affecting multiple areas🚮. Leave bin out until 21:00 if not serviced,'
                    'and then put out again by 06:30 the next day. #CTInfo #CapeTown #Refuse'
                )},
                {"role": "user", "content": (
                    '🚧 Planned Maintenance 🚧\n'
                    ...
                    'Zero-pressure testing on the water supply network. #CapeTown #Water&Sanitation'
                )},
                {"role": "assistant", "content": (
                    '🚧 Planned Maintenance 🚧\n'
                    '📍Multiple Areas\n'
            ...
            if not hashtags:
                for i, entry in enumerate(params["messages"]):
                    message = entry['content']
                    params["messages"][i]["content"] = re.sub(r'#\S+', '', message).strip()
...
class ServiceAlertAugmenter(ServiceAlertBase.ServiceAlertsBase):
    ...
    def add_social_media_posts(self, post_size_limit=280, social_media_col=TWEET_COL, hashtags=False):
        ...
        with requests.Session() as session:
            for record_index, record in zip(source_index, tqdm(json_dicts)):
                ...
                resp = _cptgpt_call_wrapper(record, session, post_size_limit, hashtags)
...
if __name__ == "__main__":
    ...
    sa_augmenter.add_social_media_posts(300, TOOT_COL, True)
